...
router.post('/add-new-rating', [upload.single('image'), uploadVideo.single('video')], async (req, res, next) => {
  try {
    ...
    // if (file) {
    //   const imageFileName = `image_${Date.now()}_${file.originalname}`;
    //   const imageUrl = await firebaseAdmin.uploadToFirebaseStorage(imageFileName, file.buffer);
    //   body = { ...body, image: imageUrl };
    // }
    // if (files) {
    //   const videoFileName = `video_${Date.now()}_${files.originalname}`;
    //   const videoUrl = await firebaseAdmin.uploadToFirebaseStorage(videoFileName, files.buffer);
  ...
  try {
    ...
    if (files) {
      ...
      if (uploadedImage) {
        ...
        const imageUrl = await firebaseAdmin.uploadToFirebaseStorage(imageFileName, uploadedImage.buffer);
        body.image = imageUrl;
      }
      if (uploadedVideo) {
        const videoFileName = `video_${Date.now()}_${uploadedVideo.originalname}`;
        const videoUrl = await firebaseAdmin.uploadToFirebaseStorage(videoFileName, uploadedVideo.buffer);
        body.video = videoUrl;
      }
